...
echo '本文を pipe で渡してもOK' | cargo run -- update message --room-id 12345678 --message-id 1234567890123456789
...
`update message` は `PUT /rooms/{room_id}/messages/{message_id}` を呼び出します。本文は `-m/--message`、`--message-file`、または標準入力からのパイプの順で取り込みます。`--dry-run` で送信内容だけを表示できます。

`delete message` は `DELETE /rooms/{room_id}/messages/{message_id}` を呼び出します。既定では削除前に確認プロンプトを表示し、`-y/--yes` でスキップできます。`--dry-run` で対象だけ表示できます。
